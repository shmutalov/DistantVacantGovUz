...
{
    public partial class OpenVacanciesWindow : Form
    {
        public OpenVacanciesWindow()
        {
            InitializeComponent();
        }
        ...
        private void RefreshVacancyList()
        {
            ...
            var preloader = new VacancyPreloader(fLoading, VacancyStatus.Open);
            workerRefreshVacancyList.RunWorkerAsync(preloader);

            fLoading.SetOperationName(Text);
            fLoading.ShowDialog();
        }
        ...
        private void toolBtnExportVacancies_Click(object sender, EventArgs e)
        {
            ...
            var exporter = new VacancyExporter(sfd.FileName, fLoading, VacancyStatus.Open);
            ...
            foreach (ListViewItem li in lstVacancies.CheckedItems)
            {
                try
                {
                    ...
                    using (var captcha = new CaptchaWindow())
                    {
                        ...
                        if (Program.VacancyApi.SetVacancyStatus(portalVacancyId, VacancyStatus.Closed))
                            statusChanged++;
                    }
                }
                catch (Exception)
        ...
        {
            var addVacancyWindow = new AddPortalVacancyWindow();

            if (addVacancyWindow.ShowDialog() == DialogResult.OK)
            {
                RefreshVacancyList();
            }
        }
